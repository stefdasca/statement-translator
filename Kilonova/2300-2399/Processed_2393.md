...
An equivalent definition of chordal graphs is the following: a graph is called a **chordal graph** if it is connected and any elementary cycle (which contains each node of the graph at most once) having at least $4$ nodes contains at least one â€œchordâ€ (an edge between two nodes that are part of the cycle but are not adjacent on the cycle). Here are some examples of chordal and non-chordal graphs:
~[img2.png]
