In ancient Rome, there are $n$ distinct senatorial settlements, one for each of the $n$ senators of the Republic. The senatorial settlements are numbered from $1$ to $n$, and between any two settlements, there exist direct or indirect connections. A connection is direct if it does not pass through other intermediate senatorial settlements. The magistrates have paved some of the direct connections between two settlements (calling such a paved connection a â€œstreetâ€) so that there exists a single sequence of streets between any two senatorial settlements by which one can travel from one settlement to the other.
